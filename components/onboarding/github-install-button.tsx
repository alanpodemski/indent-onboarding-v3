"use client"

import { useState, useCallback } from "react"
import { EASING } from "@/hooks/use-chat-engine"

type InstallState = "idle" | "loading" | "success"

export function GitHubInstallButton({ onSuccess }: { onSuccess: () => void }) {
  const [installState, setInstallState] = useState<InstallState>("idle")

  const handleInstall = useCallback(() => {
    if (installState !== "idle") return
    setInstallState("loading")
    setTimeout(() => {
      setInstallState("success")
      setTimeout(onSuccess, 600)
    }, 2000)
  }, [installState, onSuccess])

  return (
    <div
      className="pl-10 space-y-3"
      style={{ animation: `fadeInUp 400ms ${EASING.entrance} both` }}
    >
      <button
        onClick={handleInstall}
        disabled={installState !== "idle"}
        className="inline-flex h-11 items-center gap-2.5 rounded-xl border border-border bg-background px-5 text-[14px] font-medium text-foreground transition-all duration-300 hover:bg-foreground/[0.04] hover:border-foreground/20 active:scale-[0.98] disabled:pointer-events-none"
        style={{
          ...(installState === "success"
            ? { borderColor: "oklch(0.50 0.17 155)", backgroundColor: "oklch(0.50 0.17 155 / 0.08)", color: "oklch(0.45 0.15 155)" }
            : {}),
        }}
      >
        {installState === "loading" ? (
          <>
            <div className="size-5 rounded-full border-2 border-foreground/20 border-t-foreground/60 animate-spin" />
            Installing...
          </>
        ) : installState === "success" ? (
          <>
            <svg width="18" height="18" viewBox="0 0 16 16" fill="none">
              <path d="M2 9L6 13L14 3" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" />
            </svg>
            GitHub App installed
          </>
        ) : (
          <>
            <svg viewBox="0 0 24 24" fill="currentColor" className="size-5">
              <path d="M12 0C5.374 0 0 5.373 0 12c0 5.302 3.438 9.8 8.207 11.387.599.111.793-.261.793-.577v-2.234c-3.338.726-4.033-1.416-4.033-1.416-.546-1.387-1.333-1.756-1.333-1.756-1.089-.745.083-.729.083-.729 1.205.084 1.839 1.237 1.839 1.237 1.07 1.834 2.807 1.304 3.492.997.107-.775.418-1.305.762-1.604-2.665-.305-5.467-1.334-5.467-5.931 0-1.311.469-2.381 1.236-3.221-.124-.303-.535-1.524.117-3.176 0 0 1.008-.322 3.301 1.23A11.509 11.509 0 0112 5.803c1.02.005 2.047.138 3.006.404 2.291-1.552 3.297-1.23 3.297-1.23.653 1.653.242 2.874.118 3.176.77.84 1.235 1.911 1.235 3.221 0 4.609-2.807 5.624-5.479 5.921.43.372.823 1.102.823 2.222v3.293c0 .319.192.694.801.576C20.566 21.797 24 17.3 24 12c0-6.627-5.373-12-12-12z"/>
            </svg>
            Install GitHub App
          </>
        )}
      </button>

      {/* Trust statement */}
      <div className="flex items-start gap-2 text-[12px] text-muted-foreground/50 max-w-[320px]">
        <svg width="14" height="14" viewBox="0 0 16 16" fill="none" className="shrink-0 mt-0.5">
          <path d="M8 1L2 4V7.5C2 11.09 4.56 14.41 8 15.27C11.44 14.41 14 11.09 14 7.5V4L8 1Z" stroke="currentColor" strokeWidth="1.2" strokeLinecap="round" strokeLinejoin="round"/>
          <path d="M5.5 8L7 9.5L10.5 6" stroke="currentColor" strokeWidth="1.2" strokeLinecap="round" strokeLinejoin="round"/>
        </svg>
        Your code stays yours. Runs in isolated environments, never trains on your code.
      </div>
    </div>
  )
}
